...
public interface Checksumer<T> {
	...
	default Checksum checksum(T source) {
		return new Checksum(ByteChecksumer.toHexBinaryString(buildChecksum(source)));
	}
	
	byte[] buildChecksum(T source);
	
	class ByteChecksumer implements Checksumer<byte[]> {
		...
		@Override
		public byte[] buildChecksum(byte[] in) {
