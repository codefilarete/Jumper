 ...
 */
public class AddColumnHandler implements AddColumnGenerator {
	...
	public AddColumnHandler(Set<String> keywords) {
		this.keywords = keywords;
	}
	
	@Override
	public String generateScript(AddColumn modifyColumn) {
		...
		sqlAlterTable.cat(modifyColumn.getTable(), " add column ");
		generateCreateColumn(modifyColumn, sqlAlterTable);
		return sqlAlterTable.toString();
	}
	
	protected void generateCreateColumn(AddColumn column, DDLAppender sqlAlterTable) {
	...
	private class DDLAppender extends StringAppender {
		...
		public StringAppender cat(Object o) {
			if (o instanceof AddColumn) {
				String columnName = ((AddColumn) o).getName();
